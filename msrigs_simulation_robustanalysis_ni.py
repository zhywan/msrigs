...
# Component: MSRIGS Robust Analysis On Size of Identified Dataset Using Simulated Datasets
...
# Oct 15, 2020: Using function version 21.
# Oct 15, 2020: In the inner loop, add missing values and compute entropy (with missing data).
...
# configuration
id_exp = '2056_nI30000'  # ID for the set of experiments ('2056_nI10000' or '2056_nI30000')
...
save_dic: int = 0
save_S: bool = True  # save S in each iteration (only work for tested_scenario 0)
save_I: bool = True  # save I in each iteration (only work for tested_scenario 0)

# choose a parameter
parameter: int = 2  # 0:Cost of an attack. 1:Number of records in the genetic genealogy dataset.
...
pickle_filename = folder_result + 'result_p' + str(parameter) + '_ts' + str(tested_scenario) + '.pickle'
filename = folder_result + 'log_p' + str(parameter) + '_ts' + str(tested_scenario) + '.txt'
...
if __name__ == '__main__':
    ...
    for i in range(n_iter):
        ...
        for m_n in range(n_m):
            ...
            # save I
            if save_I and tested_scenario == 0:
                if not os.path.exists(folder_result + 'identified_data'):
                    os.mkdir(folder_result + 'identified_data')
                header = "YOB,State,Surname"
                np.savetxt(folder_result + 'identified_data/i' + str(i) + '_m' + str(m_n) + '.csv',
                           I[:, [2, 3, (4 + m_g)]], delimiter=',', fmt='%d', header=header, comments='')

            # Add missing values
            if missing_level > 0:
                np.random.seed(i)
                n_missing = int(n_G * m_g * missing_level)
                missed = np.append(np.zeros(n_missing), np.ones(n_G * m_g - n_missing))
