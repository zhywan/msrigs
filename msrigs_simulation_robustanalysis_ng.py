...
# Component: MSRIGS Robust Analysis On Size of Genetic Genealogy Dataset Using Simulated Datasets
...
# Oct 14, 2020: Using function version 21.
# Oct 14, 2020: In the inner loop, add missing values and compute entropy (with missing data).
# April 17, 2021: Robust analysis
...
# configuration
id_exp = '2056_nG30000'  # ID for the set of experiments ('2056_nG10000' or '2056_nG30000')
...
save_dic: int = 0
save_S: bool = False  # save S in each iteration (only work for tested_scenario 0)
save_G: bool = False  # save G in each iteration (only work for tested_scenario 0)

# choose a parameter
parameter: int = 1  # 0:Cost of an attack. 1:Number of records in the genetic genealogy dataset.
# 2:Number of records in the identified dataset.
parameter_names = ['cost', 'nG', 'nI']

# choose a scenario
scenario = 3.4  # 0: no protection. 1: no genomic data sharing. 2: random opt-in. 3: random masking. 3.x: custom masking.
# 3.1: k-anonymity. 3.4: test corresponding scenario in the standard setting.
# 4: opt-in game. 5: masking game. 6: no-attack masking game. 7: one-stage masking game.
tested_scenario = 4  # corresponding scenario to be tested
if scenario == 3.4:
    scenario_name = 'ts' + str(tested_scenario)
else:
    scenario_name = 's' + str(scenario)

# creat folders
folder_result = 'Results' + id_exp + '/' + parameter_names[parameter] + '_robust/'
if pruning == 1:
...
pickle_filename = folder_result + 'result_p' + str(parameter) + '_' + scenario_name + '.pickle'
filename = folder_result + 'log_p' + str(parameter) + '_' + scenario_name + '.txt'
...
if __name__ == '__main__':
    ...
    for i in range(n_iter):
        ...
        if missing_level > 0:
            n_G_b = 20000
            missed = np.zeros([2 * n_G_b, m_g])  # this requires: 2 * n_G_b == step * (m_n - 1) + m_start
            np.random.seed(i)
            n_missing = int(n_G_b * m_g * missing_level)
            missed_half = np.append(np.zeros(n_missing).astype(int), np.ones(n_G_b * m_g - n_missing).astype(int))
            np.random.shuffle(missed_half)
            missed_half = missed_half.reshape(n_G_b, m_g)
            missed[0:n_G_b, :] = missed_half
            missed[-n_G_b:, :] = missed_half
        for m_n in range(n_m):
            n_G = step * m_n + m_start
            print('n_G: ', n_G)
            ...
            # save S
            if save_S and tested_scenario == 0:
                if not os.path.exists(folder_result + 'target_data'):
                    os.mkdir(folder_result + 'target_data')
                header = "YOB,State"
                for j in range(m_g):
                    header += ",STR" + str(j + 1)
                np.savetxt(folder_result + 'target_data/i' + str(i) + '_m' + str(m_n) + '.csv', S[:, 2:(2 + m_g + 2)],
                           delimiter=',', fmt='%d', header=header, comments='')

            # Add missing values
            if missing_level > 0:
                G1 = np.multiply(G2[:, 4:(4 + m_g)], missed[-n_G:, :])
                G = np.concatenate((G2[:, 0:4], G1, G2[:, -2:]), axis=1)
            else:
                G = G2

            # save G
            if save_G and tested_scenario == 0:
                ...
                for j in range(m_g - 1):
                    header += ",STR" + str(j + 2)
                header += ",Surname"
                np.savetxt(folder_result + 'genealogy_data/i' + str(i) + '_m' + str(m_n) + '.csv', G[:, 4:(4 + m_g + 1)], delimiter=',',
            ...
            elif scenario == 3.4:  # robust test
                custom_strategy_filename = 'Results' + id_exp + '/Violin/m2/pruning/opt_strategy/s' + str(
                    tested_scenario) + '_i' + str(i) + '.csv'
            if scenario != 3.4:
    ...
    if save_dic == 1:
        ...
        for i in range(5):
            f1 = open(folder_result + 'dic_p' + str(parameter) + '_' + scenario_name
                      + '_' + dic_names[i] + '.pkl', 'wb')
