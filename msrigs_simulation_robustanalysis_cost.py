...
# Component: MSRIGS Sensitivity Analysis On Cost Using Simulated Datasets (Data subjects have different strategies)
...
save_dic: int = 0
save_S: bool = False  # save S in each iteration (only work for scenario 0)
save_G: bool = False  # save G in each iteration (only work for scenario 0)

# choose a parameter
parameter: int = 0  # 0:Cost of an attack. 1:Number of records in the genetic genealogy dataset.
    ...
    for i in range(n_iter):
        ...
        if save_G and scenario == 0:
            ...
            for j in range(m_g - 1):
                header += ",STR" + str(j + 2)
            header += ",Surname"
            np.savetxt(folder_result + 'genealogy_data/i' + str(i) + '.csv', G[:, 4:(4 + m_g + 1)], delimiter=',',
        ...
        if scenario == 3.1:
            ...
            custom_strategy_filename = 'Results' + id_exp + '/Violin/m2/pruning/opt_strategy/s' + str(tested_scenario)\
                                       + '_i' + str(i) + '.csv'
        if scenario != 3.4:
